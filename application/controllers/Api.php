...
class Api extends CI_Controller
{
    ...
    function getxnewsnews(){
        ...
        //exit("https://marketdataapiv3.ngxgroup.com/nsedata/api/StockPerformance?symbol=".$ccompany."&startDate=".$sdate."&endDate=" . date('Y-m-d'));
...
//        echo $result; exit;
    ...
    function getCompanyDetails($co = false){
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => "https://marketdataapiv3.ngxgroup.com/v3/api/quote/stockquotes.json?s=".$co."&_t=1b105e18ee6d464ea2e8f5bc6f66a0a6",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
    ...
    function sendMail($to = '', $code = '', $name = ''){
        ...
        //    echo $to = 'anthony@deveint.com'; $code = '333444'; $name = 'TESTNAME';
...
//CURLOPT_URL => "http://www.nse.com.ng/_api/Web/Lists/GetByTitle%28%27News%20Story%27%29/items?%24orderby=Created%20desc%2CCreated%20desc&%24top=20",
    ...
    function gettokena($returnResponse = false){
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => "https://marketdataapiv3.ngxgroup.com/nsedata/token",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
    ...
    function getDetailedQuote($token = false, $csi = false, $returnResponse = false){
        ...
        $curnArray = array(
            CURLOPT_URL => "https://marketdataapiv3.ngxgroup.com/nsedata/api/Quote?symbol=" . ($csi ? $csi : $ccompany) . "&access_token=" . ($token ? $token : $access),
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
    ...
    function getStockPerformance(){
        ...
        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_URL => "https://marketdataapiv3.ngxgroup.com/nsedata/api/StockPerformance?symbol=".$ccompany."&startDate=".$newStart."&endDate=" . $newEnd,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
    ...
    function getFactSheet(){
        ...
        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_URL => "https://marketdataapiv3.ngxgroup.com/nsedata/api/factsheet?tradingdate='".date('Y-m-d')."'",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
...
//            CURLOPT_POSTFIELDS => "{\n  \"orderBy\": \"b.boy\", \"filterType\" : \"AND\", \"id\": 155\n}",
    ...
    function getTopTradersVolumeANDTrade($orderBy = 'Trades'){
        header("Access-Control-Allow-Origin: *");
        header("Access-Control-Allow-Credentials: true");
        header("Access-Control-Allow-Methods: GET,HEAD,OPTIONS,POST,PUT");
        header("Access-Control-Allow-Headers: Access-Control-Allow-Headers, Origin,Accept, X-Requested-With, Content-Type, Access-Control-Request-Method, Access-Control-Request-Headers");
...
//        let reqData = "companyID="+this.mpya+"&DateAdded="+this.user['username'];
        ...
        $sql = $this->db->select('*')->from('trade_data')->where(array('data_title' => 'TopTradesBy' . $orderBy))->order_by('id', 'desc')->limit(1)->get();
        if($sql->num_rows() == 1){
            $sql = $sql->row();
            echo $sql->data_data;
        }else{
            $this->fetchGetTopTradersVolumeANDTrade($orderBy);
        }
    }
...
//        let reqData = "companyID="+this.mpya+"&DateAdded="+this.user['username'];
        ...
        $sql = $this->db->select('*')->from('trade_data')->where(array('data_title' => 'TopGainers'))->order_by('id', 'desc')->limit(1)->get();
        if($sql->num_rows() == 1){
            $sql = $sql->row();
            echo $sql->data_data;
        }else{
            $this->fetchGetTopGainers();
        }
    }
...
//        let reqData = "companyID="+this.mpya+"&DateAdded="+this.user['username'];
        ...
        $sql = $this->db->select('*')->from('trade_data')->where(array('data_title' => 'TopLosers'))->order_by('id', 'desc')->limit(1)->get();
        if($sql->num_rows() == 1){
            $sql = $sql->row();
            echo $sql->data_data;
        }else{
            $this->fetchGetTopLosers();
        }
    }
    ...
    function getNEWnewsXML($co = false){
        header("Access-Control-Allow-Origin: *");
        header("Access-Control-Allow-Credentials: true");
        header("Access-Control-Allow-Methods: GET,HEAD,OPTIONS,POST,PUT");
        header("Access-Control-Allow-Headers: Access-Control-Allow-Headers, Origin,Accept, X-Requested-With, Content-Type, Access-Control-Request-Method, Access-Control-Request-Headers");
...
//        let reqData = "companyID="+this.mpya+"&DateAdded="+this.user['username'];
        ...
        $sql = $this->db->select('*')->from('trade_data')->where(array('data_title' => 'NEWnewsXML'))->order_by('id', 'desc')->limit(1)->get();
        if($sql->num_rows() == 1){
            $sql = $sql->row();
            echo $sql->data_data;
        }else{
            $this->fetchGetNEWnewsXML();
        }
    }

    function getTickerData($ffiler = 'EQUITIES'){
        header("Access-Control-Allow-Origin: *");
        header("Access-Control-Allow-Credentials: true");
        header("Access-Control-Allow-Methods: GET,HEAD,OPTIONS,POST,PUT");
        header("Access-Control-Allow-Headers: Access-Control-Allow-Headers, Origin,Accept, X-Requested-With, Content-Type, Access-Control-Request-Method, Access-Control-Request-Headers");
...
//        let reqData = "companyID="+this.mpya+"&DateAdded="+this.user['username'];
        ...
        $sql = $this->db->select('*')->from('trade_data')->where(array('data_title' => 'TICKERDATA'))->order_by('id', 'desc')->limit(1)->get();
        if($sql->num_rows() == 1){
            $sql = $sql->row();
            echo $sql->data_data;
        }else{
            $this->fetchGetMarketStatus();
        }
    }
    ...
    function getMarketStatus(){
        ...
        $sql = $this->db->select('*')->from('trade_data')->where(array('data_title' => 'MarketStatus'))->order_by('id', 'desc')->limit(1)->get();
        if($sql->num_rows() == 1){
            $sql = $sql->row();
            echo $sql->data_data;
        }else{
    ...
    function gettradingdate(){
        ...
        $sql = $this->db->select('*')->from('trade_data')->where(array('data_title' => 'TradingDateData'))->order_by('id', 'desc')->limit(1)->get();
        if($sql->num_rows() == 1){
            $sql = $sql->row();
            echo $sql->data_data;
        }else{
            $this->fetchGettradingdate();
        }
    }
    ...
    function fetchGettradingdate(){
        ...
        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_URL => "https://marketdataapiv3.ngxgroup.com/nsedata/api/IndexValues?tradingdate=" . $ddate,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
        ...
        $this->db->insert('trade_data_logs', array(
            'data_title' => 'TradingDateData',
            'data_data' => $response,
            'data_count' => 0
        ));
        ...
        if ($err) {
            ...
            $keyArray = array(
                ...
                'BANK' => 'BANK',
                'NSECG' => 'NSECG',
                'NSECNSMRGDS' => 'NSEMERIVAL',
                'NSEMERIVAL' => 'NSEMERIVAL',
                'NSEGROWTH' => 'NSEGROWTH',
                'NSEINDUSTR' => 'INDUSTRIAL',
                'INDUSTRIAL' => 'INDUSTRIAL',
                'NSEINS' => 'INSURANCE',
                'INSURANCE' => 'INSURANCE',
                'NSELOTUSISLM' => 'LOTUS',
                'LOTUS' => 'LOTUS',
                'NSEMAINBOARD' => 'NSEPREMIUM',
                'NSEPREMIUM' => 'NSEPREMIUM',
                'NSEMERIGRW' => 'NSEMERIGRW',
                'NSEOILGAS' => 'OILGAS',
                'OILGAS' => 'OILGAS',
                'FOODBEV' => 'FOODBEV',
                'MARKCAP' => 'MARKCAP',
                'NSEPENSION' => 'NSEPENSION',
                'NSESOVBND' => 'NSEPREMIUM'
            );
            $newResp = array();
            $response = json_decode($response, true);
            $n = 0;
            foreach($response as $kkey => $vval){
                $vval['code'] = @$keyArray[$vval['SYMBOL']];
                $newResp[] = $vval;
                $n++;
            }

            if($n >= 5){
                $this->db->delete('trade_data', array('data_title' => 'TradingDateData'));
                $this->db->insert('trade_data', array(
                    'data_title' => 'TradingDateData',
                    'data_data' => json_encode(array('data' =>$newResp)),
                    'status' => '1',
                ));
                echo json_encode(array('data' =>$newResp));
            }
    ...
    function fetchGetMarketStatus(){
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => "https://doclib.ngxgroup.com/REST/api/statistics/mktstatus",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
        ...
        if ($err) {
            ...
            $databack = $response = json_decode($response, true);

            $closedStates = array(
                "EOD_SESS",
                "STARTOFDAY","STARTLTECP","NSE_DRPOPEN","NSE_MM_STRT","NSE_DRTRDING","CLOSE_IO","TRDENDTIME","STOPLTECP",
                "NSE_CLS_MCH",
                "NSE_CLOSE",
                "ENDOFDAY",
                "NSE_MM_STP",
...
//            "NSE_MM_STP"
            ...
            $this->db->insert('trade_data_logs', array(
                'data_title' => 'MarketStatus',
                'data_data' => json_encode($databack),
                'data_count' => 0
            ));

            $retuuu = '{}';
            if(in_array($response[0]['MktStatus'], $closedStates)){
                $retuuu = json_encode(array('ddate' => date('M, d Y'), 'status' => 'Closed'));
            }else{
                $retuuu = json_encode(array('ddate' => date('M, d Y'), 'status' => 'Open'));
            }


            $this->db->delete('trade_data', array('data_title' => 'MarketStatus'));
            $this->db->insert('trade_data', array(
                'data_title' => 'MarketStatus',
                'data_data' => $retuuu,
                'status' => '1',
    ...
    function fetchGetTopLosers(){
        ...
        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_URL => "https://marketdataapiv3.ngxgroup.com/nsedata/api/MarketMovers/Losers",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
        ...
        $originalData = $responsexxx = curl_exec($curl);
        $err = curl_error($curl);
        ...
        if ($err) {
            ...
            $this->db->insert('trade_data_logs', array(
                'data_title' => 'TopLosers',
                'data_data' => $originalData,
                'data_count' => $nn
            ));

            if($nn >= 4){
                $this->db->delete('trade_data', array('data_title' => 'TopLosers'));
                $this->db->insert('trade_data', array(
                    'data_title' => 'TopLosers',
                    'data_data' => json_encode(array('resp' => json_encode($responsexxx))),
                    'status' => '1',
                ));
            }
    ...
    function fetchGetTopGainers(){
        ...
        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_URL => "https://marketdataapiv3.ngxgroup.com/nsedata/api/MarketMovers/Gainers",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
        ...
        $originalData = $responsexxx = $ogg = curl_exec($curl);
        $err = curl_error($curl);
        ...
        if ($err) {
            ...
            $this->db->insert('trade_data_logs', array(
                'data_title' => 'TopGainers',
                'data_data' => $originalData,
                'data_count' => $nn
            ));

            if($nn >= 4){
                $this->db->delete('trade_data', array('data_title' => 'TopGainers'));
                $this->db->insert('trade_data', array(
                    'data_title' => 'TopGainers',
                    'data_data' => json_encode(array('ogg' => $ogg, 'resp' => json_encode($responsexxx))),
                    'status' => '1',
                ));
            }
    ...
    function fetchGetTopTradersVolumeANDTrade($orderBy = 'Trades'){
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => "https://doclib.ngxgroup.com/REST/api/statistics/equities/?market=&sector=&orderby=".$orderBy."&pageSize=10&pageNo=0",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
        ...
        $originalData = $response = curl_exec($curl);
        $err = curl_error($curl);
...
//            $response = '{"resp":[{"$id":"1","CalculateChangePercent":8.173076923076923,"Id":26,"Symbol":"NEM","PrevClosingPrice":2.08,"OpeningPrice":2.08,"HighPrice":2.25,"LowPrice":2.25,"ClosePrice":2.25,"Change":0.17,"PercChange":8.17,"Trades":9,"Volume":38922588,"Value":87555259.04,"Market":"Main Board","Sector":"FINANCIAL SERVICES","Company2":"NEM ","TradeDate":"2021-05-17T00:00:00"},{"$id":"2","CalculateChangePercent":0,"Id":58,"Symbol":"ZENITHBANK","PrevClosingPrice":23.7,"OpeningPrice":23.7,"HighPrice":23.7,"LowPrice":23.5,"ClosePrice":23.7,"Change":0,"PercChange":-0.42,"Trades":201,"Volume":18284812,"Value":433127570.1,"Market":"Premium Board","Sector":"FINANCIAL SERVICES","Company2":"ZENITHBANK ","TradeDate":"2021-05-17T00:00:00"},{"$id":"3","CalculateChangePercent":-7.142857142857143,"Id":53,"Symbol":"SOVRENINS","PrevClosingPrice":0.28,"OpeningPrice":0.28,"HighPrice":0.26,"LowPrice":0.26,"ClosePrice":0.26,"Change":-0.02,"PercChange":-7.14,"Trades":16,"Volume":16950000,"Value":4407000,"Market":"Main Board","Sector":"FINANCIAL SERVICES","Company2":"SOVRENINS ","TradeDate":"2021-05-17T00:00:00"},{"$id":"4","CalculateChangePercent":0.8403361344537815,"Id":30,"Symbol":"FIDELITYBK","PrevClosingPrice":2.38,"OpeningPrice":2.38,"HighPrice":2.44,"LowPrice":2.4,"ClosePrice":2.4,"Change":0.02,"PercChange":0.84,"Trades":50,"Volume":5017425,"Value":12125051.53,"Market":"Main Board","Sector":"FINANCIAL SERVICES","Company2":"FIDELITYBK ","TradeDate":"2021-05-17T00:00:00"},{"$id":"5","CalculateChangePercent":8.823529411764707,"Id":146,"Symbol":"REGALINS","PrevClosingPrice":0.34,"OpeningPrice":0.34,"HighPrice":0.37,"LowPrice":0.36,"ClosePrice":0.37,"Change":0.03,"PercChange":8.82,"Trades":20,"Volume":4106791,"Value":1511497.67,"Market":"Main Board","Sector":"FINANCIAL SERVICES","Company2":"REGALINS ","TradeDate":"2021-05-17T00:00:00"},{"$id":"6","CalculateChangePercent":5,"Id":141,"Symbol":"CHAMS","PrevClosingPrice":0.2,"OpeningPrice":0.2,"HighPrice":0.21,"LowPrice":0.21,"ClosePrice":0.21,"Change":0.01,"PercChange":5,"Trades":6,"Volume":3994346,"Value":838912.66,"Market":"Main Board","Sector":"ICT","Company2":"CHAMS ","TradeDate":"2021-05-17T00:00:00"},{"$id":"7","CalculateChangePercent":-0.591715976331361,"Id":121,"Symbol":"ACCESS","PrevClosingPrice":8.45,"OpeningPrice":8.45,"HighPrice":8.45,"LowPrice":8.4,"ClosePrice":8.4,"Change":-0.05,"PercChange":-0.59,"Trades":94,"Volume":3837986,"Value":32268148.7,"Market":"Premium Board","Sector":"FINANCIAL SERVICES","Company2":"ACCESS ","TradeDate":"2021-05-17T00:00:00"},{"$id":"8","CalculateChangePercent":-0.7042253521126761,"Id":148,"Symbol":"FBNH","PrevClosingPrice":7.1,"OpeningPrice":7.1,"HighPrice":7.1,"LowPrice":7.05,"ClosePrice":7.05,"Change":-0.05,"PercChange":-0.7,"Trades":47,"Volume":3386860,"Value":23966768.65,"Market":"Premium Board","Sector":"FINANCIAL SERVICES","Company2":"FBNH ","TradeDate":"2021-05-17T00:00:00"},{"$id":"9","CalculateChangePercent":0,"Id":102,"Symbol":"TRANSCORP","PrevClosingPrice":0.88,"OpeningPrice":0.88,"HighPrice":0.88,"LowPrice":0.88,"ClosePrice":0.88,"Change":0,"PercChange":null,"Trades":23,"Volume":2930692,"Value":2579640.08,"Market":"Main Board","Sector":"CONGLOMERATES","Company2":"TRANSCORP ","TradeDate":"2021-05-17T00:00:00"},{"$id":"10","CalculateChangePercent":-2.4,"Id":21,"Symbol":"UCAP","PrevClosingPrice":6.25,"OpeningPrice":6.25,"HighPrice":6.4,"LowPrice":6.1,"ClosePrice":6.1,"Change":-0.15,"PercChange":-2.4,"Trades":48,"Volume":2556733,"Value":16196693,"Market":"Main Board","Sector":"FINANCIAL SERVICES","Company2":"UCAP ","TradeDate":"2021-05-17T00:00:00"}]}';
            $antooo = json_decode($response, true);


            $nn = strlen(json_encode(array('resp' => $antooo)));
            if(strlen($originalData) >= 1000){

                $zeroChange = 0;
                foreach ($antooo as $kkey => $vval){
                    if($vval['CalculateChangePercent'] == 0){
                        $zeroChange++;
                    }
                }
                ...
                if($zeroChange <= 3){
                    $this->db->insert('trade_data_logs', array(
                        'data_title' => 'TopTradesBy' . $orderBy,
                        'data_data' => $originalData,
                        'data_count' => $nn
                    ));

                    $this->db->delete('trade_data', array('data_title' => 'TopTradesBy' . $orderBy));
                    $this->db->insert('trade_data', array(
                        'data_title' => 'TopTradesBy' . $orderBy,
                        'data_data' => json_encode(array('resp' => $antooo)),
                        'status' => '1',
                    ));
                }else{
                    echo "no data saved";
            ...
            echo json_encode(array('resp' => $antooo));
    ...
    function fetchGetNEWnewsXML($co = false){
        ...
        curl_setopt_array($curl, array(
//            CURLOPT_URL => "https://ngxgroup.com/feed/",
            CURLOPT_URL => "https://ngxgroup.com/category/exchange/feed/",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
        ...
        if ($err) {
            ...
            $mantooo = json_decode(json_encode(simplexml_load_string($response)), true);

            if(strlen(json_encode(array('resp' => $mantooo['channel']['item']))) >= 50) {
                $this->db->delete('trade_data', array('data_title' => 'NEWnewsXML'));
                $this->db->insert('trade_data', array(
                    'data_title' => 'NEWnewsXML',
                    'data_data' => json_encode(array('resp' => $mantooo['channel']['item'])),
                    'status' => '1',
                ));
            }
            echo json_encode(array('resp' => $mantooo['channel']['item']));
        }
    ...
    function fetchGetTickerData($ffiler = 'EQUITIES'){
        header("Access-Control-Allow-Origin: *");
        header("Access-Control-Allow-Credentials: true");
        header("Access-Control-Allow-Methods: GET,HEAD,OPTIONS,POST,PUT");
        header("Access-Control-Allow-Headers: Access-Control-Allow-Headers, Origin,Accept, X-Requested-With, Content-Type, Access-Control-Request-Method, Access-Control-Request-Headers");
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => "https://doclib.ngxgroup.com/REST/api/statistics/ticker?filter=TickerType%20%20eq%20%27".$ffiler."%27",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
            CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
            CURLOPT_CUSTOMREQUEST => "GET",
            CURLOPT_POSTFIELDS => "name=Mutisyaa%20Employee&email=anthony%40superstar.com&phone=0724234433&password=11111111&biz_id=1&primaryAccount=NO&status=ACTIVE&username=supermutisya&userType=SUPERSTAR",
            CURLOPT_HTTPHEADER => array(
                "accept: application/json",
                "authorization: Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9.eyJhdWQiOiIzIiwianRpIjoiMThkMzA4MmYxYjI4ODU3NmE2ZjJhNmQ4YzhmZGIwMmY4ZTJhMDY2ZWViMjAyNjNhZjVhNTg0MzU5M2RkY2IyZWUxNDNjMmQ2NGU1MjM4ZWMiLCJpYXQiOjE2MTg1MDkxOTAsIm5iZiI6MTYxODUwOTE5MCwiZXhwIjoxNjUwMDQ1MTkwLCJzdWIiOiIxNyIsInNjb3BlcyI6W119.EaRvds2nGqYLcO7FegU1LqUNnrxzHEWD1kViN1xEw-g9wrK0oxzaZrDX8Ua-28YMl4M4I6kyxw9XmqWM_V76eCY4BCdTsRCIEOJCo-CiTvzEgtmqkOQl3UiYWudzWMkbNQp8V1UwhewnK1PofXj51gbyD8DcI99WqZcicbNKohRAzZKa2cCDA_JupI9aWQX75VgaYZTFklJ4kTHK2qBnhJeLHILF7Uafm_Oyo8fg9WsPqtDPL_eiKnJAM19J9XO-mOK4NFctBrbVPtIqDvGmv5m03XsccJIEPnc4gd_vjrJnwldtw4zXaudz19lBgUyTgU373fo1HSu8zMwPlq6jtO5-GmkdmJIS7_LkidAPwMbKG8qqO0CF74ocYAtcSERr-zJFlq0ULiJ9rONoSsylvmRQmrWdQO4870RPa2Fe0YSnopQB8nouVNu8Co72OKkqDMUKlO7tPtzldclOAQO6wirGIBCGrSjOJ8ijySOr6g9EYutU1UBntcBse2p9ospFP1eKNRnLrG8RKXQpc3ZJO1hJoGzb49cxsE9uBr3KyavAprNDeZl-cJWSCzHr7m82XXz7f4kj3n7M5UYaF1khYaPogAoLVgL4aboz6VHZpCT3bH4OWcMCz-_ApuuHuWYcSbCJJh4tJlClmHBua6jwEhtDgfMsFgD8G3XpdO1QiDY",
                "cache-control: no-cache",
                "postman-token: 4840b448-a628-9552-8fe8-163d550104ef"
            ),
        ));
        ...
        if ($err) {
            ...
            if(strlen($originalData) >= 1000){
                ...
                $this->db->insert('trade_data_logs', array(
                    'data_title' => 'TICKERDATA',
                    'data_data' => $originalData,
                    'data_count' => 0
                ));


                $this->db->delete('trade_data', array('data_title' => 'TICKERDATA'));
                $this->db->insert('trade_data', array(
                    'data_title' => 'TICKERDATA',
                    'data_data' => json_encode(array('resp' => $response)),
                    'status' => '1',
                ));
            }
            echo json_encode(array('resp' => $response));
